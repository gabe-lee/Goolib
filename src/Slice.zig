...
const ERR_CANNOT_INCREASE_START = LOG_PREFIX ++ "Slice START_MUTABILITY != .increase_only or .increase_or_decrease, operation would increase start address";
const ERR_CANNOT_DECREASE_START = LOG_PREFIX ++ "Slice START_MUTABILITY != .decrease_only or .increase_or_decrease, operation would decrease start address";
const ERR_CANNOT_INCREASE_END = LOG_PREFIX ++ "Slice END_MUTABILITY != .increase_only or .increase_or_decrease, operation would increase end address";
const ERR_CANNOT_DECREASE_END = LOG_PREFIX ++ "Slice END_MUTABILITY != .decrease_only or .increase_or_decrease, operation would decrease end address";
const ERR_CANNOT_GROW_LEN = LOG_PREFIX ++ "Slice LEN_MUTABILITY != .grow_only or .shrink_or_grow, operation would grow length";
const ERR_CANNOT_SHRINK_LEN = LOG_PREFIX ++ "Slice LEN_MUTABILITY != .shrink_only or .shrink_or_grow, operation would shrink length";
const ERR_OPERATE_IMMUTABLE_ELEM = LOG_PREFIX ++ "Slice(ELEM_MUTABILITY = .immutable) attempted to change element value";
const ERR_OPERATE_NULL = LOG_PREFIX ++ "Slice: cannot operate on null ptr";
const ERR_SHRINK_OOB = LOG_PREFIX ++ "Slice: shrink count ({d}) would cause condition `first_address > last_address` (max shrink = len = {d})";
const ERR_START_END_REVERSED = LOG_PREFIX ++ "Slice: provided start ({d}) and end ({s}) indexes would cause condition `first_address > last_address`";
const ERR_INDEX_OOB = LOG_PREFIX ++ "Slice: the largest requested or provided index ({d}) is out of slice bounds (len = {d})";
const ERR_INDEX_CHUNK_OOB = LOG_PREFIX ++ "Slice: requested or provided start + count ({d} + {d} = {d}) would put the resulting sub-slice out of original bounds (len = {d})";
const ERR_SHIFT_OVERLAP = LOG_PREFIX ++ "Slice: a `shift({s}) -> @memcopy` operation isn't shifted far enough to guarantee no overlap (min_shift = len = {d})";

pub fn Slice(comptime T: type, comptime Idx: type, ELEM_MUTABILITY: Mutability) type {
    ...
    return extern struct {
        ...
        pub inline fn to_zig_slice(self: Self) if (MUTABLE) []T else []const T {
            assert_with_reason(self.ptr != null, LOG_PREFIX ++ "cannot cast Slice({s}).ptr == `null` to native zig slice []{s}", .{ @typeName(T), @typeName(T) });
            return self.ptr.?[0..@intCast(self.len)];
        }

        pub inline fn to_nullable_zig_slice(self: Self) if (MUTABLE) ?[]T else ?[]const T {
            if (self.ptr == null) return null;
            return self.ptr.?[0..@intCast(self.len)];
        }

        pub inline fn change_mutability(self: Self, comptime elem_mutability: Mutability) Slice(T, Idx, elem_mutability) {
            if (elem_mutability == ELEM_MUTABILITY) return self;
            return Slice(T, Idx, elem_mutability){
        ...
        pub inline fn new_with_start_end(ptr: [*]T, start: Idx, end: Idx) Self {
            assert_with_reason(end >= start, ERR_START_END_REVERSED, .{ start, end });
            return Self{ .ptr = ptr + start, .len = end - start };
        }

        pub inline fn grow_right(self: Self, count: Idx) Self {
            assert_with_reason(self.ptr != null, ERR_OPERATE_NULL, .{});
        ...
        pub inline fn shrink_right(self: Self, count: Idx) Self {
            assert_with_reason(self.ptr != null, ERR_OPERATE_NULL, .{});
            assert_with_reason(self.len >= count, ERR_SHRINK_OOB, .{ count, self.len });
            return Self{ .ptr = self.ptr, .len = self.len - count };
        }
        ...
        pub inline fn sub_slice_start_len(self: Self, start: Idx, len: Idx) Self {
            assert_with_reason(self.ptr != null, ERR_OPERATE_NULL, .{});
            assert_with_reason(start + len <= self.len, ERR_INDEX_CHUNK_OOB, .{ start, len, start + len, self.len });
        ...
        pub inline fn sub_slice_start_end(self: Self, start: Idx, end: Idx) Self {
            assert_with_reason(end >= start, ERR_START_END_REVERSED, .{ start, end });
            assert_with_reason(self.ptr != null, ERR_OPERATE_NULL, .{});
            assert_with_reason(end <= self.len, ERR_INDEX_CHUNK_OOB, .{ start, end - start, end, self.len });
            return Self{ .ptr = self.ptr.? + start, .len = end - start };
        }

        pub inline fn sub_slice_from_start(self: Self, len: Idx) Self {
            assert_with_reason(self.ptr != null, ERR_OPERATE_NULL, .{});
            assert_with_reason(len <= self.len, ERR_INDEX_CHUNK_OOB, .{ 0, len, len, self.len });
            return Self{ .ptr = self.ptr, .len = len };
        }

        pub inline fn sub_slice_from_end(self: Self, len: Idx) Self {
            assert_with_reason(self.ptr != null, ERR_OPERATE_NULL, .{});
            assert_with_reason(len <= self.len, ERR_INDEX_CHUNK_OOB, .{ self.len - len, len, self.len, self.len });
        ...
        pub inline fn get_item_ptr(self: Self, idx: Idx) *T {
            assert_with_reason(self.ptr != null, ERR_OPERATE_NULL, .{});
            assert_with_reason(idx < self.len, ERR_INDEX_OOB, .{ idx, self.len });
            return &self.ptr.?[idx];
        }

        pub inline fn get_last_item_ptr(self: Self) *T {
            assert_with_reason(self.ptr != null, ERR_OPERATE_NULL, .{});
            assert_with_reason(self.len > 0, ERR_INDEX_OOB, .{ -1, self.len });
            return &self.ptr.?[self.len - 1];
        }

        pub inline fn get_first_item_ptr(self: Self) *T {
            assert_with_reason(self.ptr != null, ERR_OPERATE_NULL, .{});
            assert_with_reason(self.len > 0, ERR_INDEX_OOB, .{ 0, self.len });
            return &self.ptr.?[0];
        }
        ...
        pub fn copy_rightward(self: Self, count: Idx) Self {
            ...
            if (count > self.len) {
                @memcpy(new_slice.to_zig_slice(), self.to_zig_slice());
            } else {
                std.mem.copyBackwards(T, new_slice.to_zig_slice(), self.to_zig_slice());
            }
            return new_slice;
        }
        ...
        pub fn copy_leftward(self: Self, count: Idx) Self {
            ...
            if (count > self.len) {
                ...
                std.mem.copyForwards(T, new_slice.to_zig_slice(), self.to_zig_slice());
            }
            return new_slice;
        }
        ...
        pub fn sorted_binary_search_insert_idx_for_item(self: Self, item: T) Idx {
            return @intCast(BinarySearch.binary_search_insert_index(T, item, self.to_zig_slice()));
        }
        ...
        pub fn sorted_binary_search_insert_idx_for_item_with_transform(self: Self, item: T, comptime TX: type, transform_fn: *const fn (item: T) TX) Idx {
            return @intCast(BinarySearch.binary_search_insert_index_with_transform(T, item, self.to_zig_slice(), TX, transform_fn));
        }
        ...
        pub fn sorted_binary_search_insert_idx_for_item_with_transform_and_user_data(self: Self, item: T, comptime TX: type, transform_fn: *const fn (item: T, user_data: ?*anyopaque) TX, user_data: ?*anyopaque) Idx {
            return @intCast(BinarySearch.binary_search_insert_index_with_transform_and_user_data(T, item, self.to_zig_slice(), TX, transform_fn, user_data));
        }
    };
}
