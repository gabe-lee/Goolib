...
pub fn define_id(comptime details: IdDetails) type {
    assert_with_reason(details.index_bits != 0, @src(), "index bits cannot be zero", .{});
    assert_with_reason(Types.type_is_unsigned_int(details.raw_type), @src(), "raw type must be an unsigned integer type that can hold index_bits + generation_bits", .{});
    assert_with_reason(@bitSizeOf(details.raw_type) >= details.generation_bits + details.index_bits, @src(), "raw type must be an unsigned integer type that can hold index_bits + generation_bits", .{});
    return packed struct {
        raw: Raw,

        const Id = @This();
        const HAS_GEN = details.generation_bits != 0;
        const GEN_OFFSET = details.index_bits;
        const IDX_MASK: Raw = if (HAS_GEN) (@as(Raw, 1) << GEN_OFFSET) - 1 else math.maxInt(Raw);
        const IDX_CLEAR: Raw = if (HAS_GEN) ~IDX_MASK else 0;
        const GEN_MASK: Raw = if (HAS_GEN) ~IDX_MASK else 0;
        const GEN_CLEAR: Raw = if (HAS_GEN) IDX_MASK else 0;
        const GEN_ONE: Raw = if (HAS_GEN) @as(Raw, 1) << GEN_OFFSET else 1;
        pub const Raw = details.raw_type;
        pub const Gen = if (HAS_GEN) meta.Int(.unsigned, details.generation_bits) else u0;
        pub const Idx = meta.Int(.unsigned, details.index_bits);
        var print_buf: [64]u8 = undefined;

        pub inline fn equals(self: Id, other: Id) bool {
            return self.raw == other.raw;
        }
        pub inline fn gen(self: Id) Gen {
            if (!HAS_GEN) return 0;
            return @intCast((self.raw & GEN_MASK) >> GEN_OFFSET);
        }
        pub inline fn same_gen(self: Id, other: Id) bool {
            if (!HAS_GEN) return true;
            return (self.raw & GEN_MASK) == (other.raw & GEN_MASK);
        }
        pub inline fn same_index(self: Id, other: Id) bool {
            return (self.raw & IDX_MASK) == (other.raw & IDX_MASK);
        }
        pub inline fn diff_index(self: Id, other: Id) bool {
            return (self.raw & IDX_MASK) != (other.raw & IDX_MASK);
        }
        pub inline fn idx(self: Id) Idx {
            return @intCast(self.raw & IDX_MASK);
        }
        pub inline fn not_null(self: Id) bool {
            return self.raw & NULL_MASK_NONE != NULL_MASK_NONE;
        }
        pub inline fn is_null(self: Id) bool {
            return self.raw & NULL_MASK_NONE == NULL_MASK_NONE;
        }
        pub inline fn is_null_parent_is_root(self: Id) bool {
            return self.raw == NULL_MASK_ROOT;
        }
        pub inline fn is_null_none(self: Id) bool {
            return self.raw == NULL_MASK_NONE;
        }
        // inline fn increase_gen(self: *Id) void {
        //     self.raw += GEN_ONE;
        //     if (self.raw * GEN_MASK == NULL_GEN) {
        //         self.raw |= GEN_CLEAR;
        //     }
        // }
        pub fn format(self: Id, fmt: []const u8, opts: std.fmt.FormatOptions, writer: anytype) !void {
            _ = fmt;
            _ = opts;
            if (self.is_null()) {
                switch (self.raw) {
                    NULL_MASK_ROOT => try writer.writeAll("(null (on root))"),
                    NULL_MASK_NONE => try writer.writeAll("(null (none))"),
                    else => unreachable,
                }
            } else {
                try writer.print("(idx = {d}, gen = {d})", .{ self.idx(), self.gen() });
            }
        }

        const NULL_MASK_NONE: Raw = math.maxInt(Raw) - 1;
        const NULL_MASK_ROOT: Raw = math.maxInt(Raw);

        const NULL_ON_ROOT = Id{ .raw = NULL_MASK_ROOT };
        const NULL_NONE = Id{ .raw = math.maxInt(Raw) - 1 };
    };
}
...
pub const ForwardLinkedHeirarchyOptions = struct {
    ...
    /// This is used to verify the types of the id fields on the user element,
    /// which MUST be the same type generated by `define_id(element_id_details)`
    ///
    /// If `generation_bits` is not zero, `own_id_field` must also not be null,
    /// and the following behavior is added:
    ///
    /// Any time an item is sent to the 'free' pool it's generation
    ...
    /// possibility that user code holding an id with an old generation is once again a 'valid'
...
pub const IdDetails = struct {
    index_bits: usize,
    generation_bits: usize,
    raw_type: type,
};
...
pub fn LinkedHeirarchy(comptime options: ForwardLinkedHeirarchyOptions) type {
    ...
    const ID = define_id(options.element_id_details);
    var linkage_count = assert_field_type_matches_id_type(E, ID, options.next_field);
    linkage_count += assert_field_type_matches_id_type(E, ID, options.prev_field);
    linkage_count += assert_field_type_matches_id_type(E, ID, options.parent_field);
    linkage_count += assert_field_type_matches_id_type(E, ID, options.first_left_child_field);
    linkage_count += assert_field_type_matches_id_type(E, ID, options.first_right_child_field);
    linkage_count += assert_field_type_matches_id_type(E, ID, options.last_left_child_field);
    linkage_count += assert_field_type_matches_id_type(E, ID, options.last_right_child_field);
    ...
    if (options.element_id_details.generation_bits != 0) {
        assert_with_reason(options.own_id_field != null, @src(), "`options.element_id_details.generation_bits` > 0 can only be used when `options.own_id_field` is not null", .{});
    }
    assert_with_reason(options.element_id_details.index_bits == @bitSizeOf(options.element_memory_options.index_type), @src(), "`options.element_id_details.index_bits` MUST equal `@bitSizeOf(options.element_memory_options.index_type)`", .{});

    return struct {
        list: List = List.UNINIT,
        first_free_id: Id = Id.NULL_NONE,
        free_count: Index = 0,
        first_root_id: Id = Id.NULL_NONE,
        last_root_id: if (HAS_LAST_ROOT) Id else void = if (HAS_LAST_ROOT) Id.NULL_NONE else void{},
        ...
        const UNINIT_ELEM: Elem = make: {
            ...
            Internal.set_own_id(&elem, Id.NULL_NONE);
            Internal.set_next_id(&elem, Id.NULL_NONE);
            Internal.set_prev_id(&elem, Id.NULL_NONE);
            Internal.set_parent_id(&elem, Id.NULL_NONE);
            Internal.set_first_left_child_id(&elem, Id.NULL_NONE);
            Internal.set_last_left_child_id(&elem, Id.NULL_NONE);
            Internal.set_first_right_child_id(&elem, Id.NULL_NONE);
            Internal.set_last_right_child_id(&elem, Id.NULL_NONE);
            break :make elem;
        };
        const HAS_GEN = Id.HAS_GEN;
        ...
        const HAS_LEFT_CHILD_NODES = HAS_FIRST_LEFT_CHILD or HAS_LAST_LEFT_CHILD;
        const HAS_RIGHT_CHILD_NODES = HAS_FIRST_RIGHT_CHILD or HAS_LAST_RIGHT_CHILD;
        ...
        const HAS_ANY_CHILD_NODES = HAS_FIRST_CHILD_NODES or HAS_LAST_CHILD_NODES;
        ...
        //     gen: if (HAS_GEN) Id.Raw else void = if (HAS_GEN) NULL_GEN else void{},
        // };
        ...
        //     id_ptr: *Id.Raw = &DUMMY_ID,
        //     id: Id.Raw = Id.NULL_NONE,
        // };
        const Range = struct {
            first: Id = Id.NULL_NONE,
            last: Id = Id.NULL_NONE,
        };
        // const FirstLastParent = struct {
        //     first: Id = Id.NULL_NONE,
        //     last: Id = Id.NULL_NONE,
        //     parent: Id = Id.NULL_NONE,
        // };
        ...
        var DUMMY_ID: Id = Id.NULL_NONE;
        // var DUMMY_ELEM: Elem = undefined;

        pub inline fn get_id(ptr: *const Elem) Id {
            Internal.assert_has_own_id(@src());
            return @bitCast(@field(ptr, OWN_ID_FIELD));
        }
        pub inline fn get_id_ptr(ptr: *const Elem) *Id {
            Internal.assert_has_own_id(@src());
            return @ptrCast(&@field(ptr, OWN_ID_FIELD));
        }
        ...
        pub inline fn get_ptr(self: *Heirarchy, id: Id) *Elem {
            ...
            if (HAS_GEN) {
                const own_id = get_id(ptr);
                assert_with_reason(id.same_gen(own_id), @src(), "generation on id requested (index {d}, gen {d}) did not match the generation stored on element {d}", .{id.idx(), id.gen(), own_id.gen()});
            }
            return ptr;
        }
        ...
        pub const Internal = struct {
            ...
            pub fn swap_data_only(a: *Elem, b: *Elem) void {
                if (COPY_VAL_FN) |copy| {
                    ...
                    if (HAS_OWN_ID) {
                        set_own_id(a, get_id(old_a));
                        set_own_id(b, get_id(old_b));
                    }
                }
            }

            pub fn initialize_new_index(self: *Heirarchy) Id {
                const new_id: Id = Id{.raw = @intCast(self.list.len)};
                var new_elem = UNINIT_ELEM;
                set_own_id(&new_elem, new_id);
                _ = self.list.append(new_elem, null);
                return new_id;
            }

            pub fn initialize_new_indexes_as_siblings(self: *Heirarchy, count: Index) Range {
                assert_has_next(@src());
                const first_id: Id.Raw = @intCast(self.list.len);
                const last_id: Id.Raw = first_id + @as(Id.Raw, @intCast(count - 1));
                var prev_id: Id.Raw = Id.NULL_NONE.raw;
                var this_id: Id.Raw = first_id;
                _ = self.list.append_n_times(UNINIT_ELEM, count, null);
                while (this_id < last_id) {
                    const this_ptr = Internal.get_ptr_ignore_gen(self, @bitCast(this_id));
                    set_own_id(this_ptr, @bitCast(this_id));
                    if (HAS_PREV) set_prev_id(this_ptr, @bitCast(prev_id));
                    prev_id = this_id;
                    this_id += 1;
                    if (HAS_NEXT) set_next_id(this_ptr, @bitCast(this_id));
                }
                const this_ptr = Internal.get_ptr_ignore_gen(self, @bitCast(this_id));
                if (HAS_PREV) set_prev_id(this_ptr, @bitCast(prev_id));
                set_own_id(this_ptr, @bitCast(this_id));
                return Range{
                    .first = @bitCast(first_id),
                    .last = @bitCast(last_id),
                };
            }

            pub fn pop_free_item(self: *Heirarchy) Id.Raw {
                assert_with_reason(self.free_count > 0 and self.first_free_id.not_null(), @src(), "no free items to pop", .{});
                const id = self.first_free_id;
                const ptr = self.get_ptr(id);
                const free_ptr = &@field(ptr, FREE_FIELD);
                const next_free = free_ptr.*;
                free_ptr.* = Id.NULL_NONE;
            ...
            pub fn push_free_item(self: *Heirarchy, id: Id) void {
                assert_with_reason(id.not_null(), @src(), "cannot push 'null' item to free list", .{});
                const ptr = self.get_ptr(id);
                if (options.element_memory_options.secure_wipe_bytes) {
                    ptr.* = UNINIT_ELEM;
                } else {
                    if (!FREE_IS_FIRST_LEFT) set_first_left_child_id(ptr, Id.NULL_NONE);
                    if (!FREE_IS_LAST_LEFT) set_last_left_child_id(ptr, Id.NULL_NONE);
                    if (!FREE_IS_FIRST_RIGHT) set_first_right_child_id(ptr, Id.NULL_NONE);
                    if (!FREE_IS_LAST_RIGHT) set_last_right_child_id(ptr, Id.NULL_NONE);
                    if (!FREE_IS_NEXT) set_next_id(ptr, Id.NULL_NONE);
                    if (!FREE_IS_PREV) set_prev_id(ptr, Id.NULL_NONE);
                    if (!FREE_IS_PARENT) set_parent_id(ptr, Id.NULL_NONE);
                }
                const free_ptr = &@field(ptr, FREE_FIELD);
            ...
            pub fn pop_and_initialize_free_items_as_siblings(self: *Heirarchy, count: Index) Range {
                ...
                var prev_id = Id.NULL_NONE;
                const limit = count - 1;
                while (c < limit) : (c += 1) {
                    const next_id = pop_free_item(self);
                    const this_ptr = self.get_ptr(this_id);
                    if (HAS_PREV) set_prev_id(this_ptr, prev_id);
                    prev_id = this_id;
                    if (HAS_NEXT) set_next_id(this_ptr, next_id);
                    this_id = next_id;
                }
                if (HAS_PREV) {
                    const this_ptr = self.get_ptr(this_id);
                    set_prev_id(this_ptr, prev_id);
                }
                return Range{
                    .first = first_free,
                    .last = this_id,
            ...
            pub fn initialize_one_item(self: *Heirarchy) Id.Raw {
            ...
            pub fn remove_id_range(self: *Heirarchy, leftmost: Id, first_removed_id: Id, last_removed_id: Id, rightmost: Id, known_parent_id: ?Id) void {
                //FIXME logic requiring parents is flawed somehow, refactor this func
                assert_with_reason(first_removed_id.not_null() and last_removed_id.not_null(), @src(), "cannot remove 'null' ids between items left{}, right{}", .{leftmost, rightmost});
                Internal.assert_range_and_ends_adjacent_siblings(self, leftmost, first_removed_id, last_removed_id, rightmost, @src());
                const branch = Utils.bools_to_switchable_integer(2, .{leftmost.not_null(), rightmost.not_null()});
                const parent_id = get: {
                    if (HAS_PARENT) {
                        const parent = self.get_parent_id(self.get_ptr(first_removed_id));
                        if (known_parent_id) |k_parent| {
                            assert_with_reason(parent.equals(k_parent), @src(), "cached parent on 'first removed id' {any} did not match the provided 'known parent id' {any}", .{parent, k_parent});
                        }
                        break :get parent;
                    } else break :get Id.NULL_ON_ROOT;
                };
                switch (branch) {
                    0 => {// left and right == NULL_ID
                        if (HAS_DEPTH) {
                            ...
                            if (parent_id.is_null()) {
                                if (parent_id.is_null_parent_is_root()) {
                                    self.first_root_id = Id.NULL_NONE;
                                    if (HAS_LAST_ROOT) self.last_root_id = Id.NULL_NONE;
                                }
                            } else {
                                const parent_ptr = self.get_ptr(parent_id);
                                var found: usize = 0;
                                if (HAS_FIRST_LEFT_CHILD) {
                                    const cached_first_left = get_first_left_child_id(parent_ptr);
                                    if (cached_first_left.equals(first_removed_id)) {
                                        Internal.set_first_left_child_id(parent_ptr, Id.NULL_NONE);
                                        Internal.set_last_left_child_id(parent_ptr, Id.NULL_NONE);
                                        found += 1;
                                    }
                                } else if (HAS_LAST_LEFT_CHILD) {
                                    const cached_last_left = get_last_left_child_id(parent_ptr);
                                    if (cached_last_left.equals(last_removed_id)) {
                                        Internal.set_first_left_child_id(parent_ptr, Id.NULL_NONE);
                                        Internal.set_last_left_child_id(parent_ptr, Id.NULL_NONE);
                                        found += 1;
                                    }
                                ...
                                if (HAS_FIRST_RIGHT_CHILD) {
                                    const cached_first_right = get_first_right_child_id(parent_ptr);
                                    if (cached_first_right.equals(first_removed_id)) {
                                        Internal.set_first_right_child_id(parent_ptr, Id.NULL_NONE);
                                        Internal.set_last_right_child_id(parent_ptr, Id.NULL_NONE);
                                        found += 1;
                                    }
                                } else if (HAS_LAST_RIGHT_CHILD) {
                                    const cached_last_right = get_last_right_child_id(parent_ptr);
                                    if (cached_last_right.equals(last_removed_id)) {
                                        Internal.set_first_right_child_id(parent_ptr, Id.NULL_NONE);
                                        Internal.set_last_right_child_id(parent_ptr, Id.NULL_NONE);
                                        found += 1;
                                    }
                                }
                                assert_with_reason(found > 0, @src(), "did not find first or last item in range on parent {any} connections", .{parent_id});
                            }
                        } else {
                            self.first_root_id = Id.NULL_NONE;
                            if (HAS_LAST_ROOT) self.last_root_id = Id.NULL_NONE;
                    ...
                    1 => {// left valid, right == NULL_ID
                        ...
                        if (HAS_PREV) {
                            ...
                            Internal.set_prev_id(first_ptr, Id.NULL_NONE);
                        }
                        if (HAS_DEPTH) {
                            Internal.assert_has_parent(@src());
                            if (parent_id.is_null()) {
                    ...
                    2 => {// right valid, left == NULL_ID
                        if (HAS_NEXT) {
                            ...
                            Internal.set_next_id(last_ptr, Id.NULL_NONE);
                        }
                        if (HAS_PREV) {
                            const right_ptr = self.get_ptr(rightmost);
                ...
                if (HAS_PARENT) {
                    Internal.set_parent_on_range(self, Id.NULL_DISCONN, Id.NULL_DISCONN, first_removed_id, last_removed_id, Id.NULL_DISCONN);
                }
            }
            ...
            pub fn assert_indexes_not_equal(a: Id, b: Id, comptime src_loc: ?SourceLocation) void {
                assert_with_reason(a.diff_index(b), src_loc, "expected two different indexes, got {s} and {s}", .{a, b});
            }
            ...
            pub fn assert_real_next_cached_next_match(self: *Heirarchy, this_id: Id, real_next: Id, comptime src_loc: ?SourceLocation) void {
                ...
                assert_with_reason(real_next == cached_next, src_loc, "real next id {any} does not match the cached next id {any} on the prev sibling {any}", .{ real_next, cached_next, this_id });
            }

            pub fn assert_has_own_id(comptime src_loc: ?SourceLocation) void {
                assert_with_reason(HAS_OWN_ID, src_loc, "items do not cache their own id", .{});
            }
            ...
            pub fn assert_range_and_ends_adjacent_siblings(self: *Heirarchy, leftmost: Id, range_first: Id, range_last: Id, rightmost: Id, comptime src_loc: ?SourceLocation) void {
                if (HAS_NEXT) {
                    const right_cached = get_next_id(self.get_ptr(range_last));
                    assert_with_reason(right_cached.equals(rightmost), src_loc, "last item in range {any} did not point to rightmost item {any}", .{range_last, rightmost});
                    if (leftmost.not_null()) {
                        const first_cached = get_next_id(self.get_ptr(leftmost));
                        assert_with_reason(first_cached.equals(range_first), src_loc, "leftmost item {any} did not point to first item in range {any}", .{leftmost, range_first});
                    }
                }
                if (HAS_PREV) {
                    const left_cached = get_prev_id(self.get_ptr(range_first));
                    assert_with_reason(left_cached.equals(leftmost), src_loc, "first item in range {any} did not point to leftmost item {any}", .{range_last, leftmost});
                    if (rightmost.not_null()) {
                        const last_cached = get_prev_id(self.get_ptr(rightmost));
                        assert_with_reason(last_cached.equals(range_last), src_loc, "rightmost item {any} did not point to last item in range {any}", .{rightmost, range_last});
                    }
                }
            }

            pub fn assert_disconnect_range_is_isolated(self: *Heirarchy, range: Range, comptime src_loc: ?SourceLocation) void {
                if (HAS_NEXT) assert_with_reason(get_next_id(self.get_ptr(range.last)).is_null(), src_loc, "last disconnected item {any} did not have a 'next' field that pointed to NULL_ID, it wasnt fully disconnected", .{range.last});
                if (HAS_PREV) assert_with_reason(get_prev_id(self.get_ptr(range.first)).is_null(), src_loc, "first disconnected item {any} did not have a 'prev' field that pointed to NULL_ID, it wasnt fully disconnected", .{range.first});
            }

            // pub fn assert_2_siblings_in_order_no_cycles(self: *Heirarchy, a: Id.Raw, b: Id.Raw, comptime a_b_can_equal: bool, comptime b_can_be_null: bool, comptime src_loc: ?SourceLocation) void {
            ...
            // pub fn assert_4_siblings_in_order_no_cycles_b_c_can_be_same_d_can_be_null(self: *Heirarchy, a: Id.Raw, b: Id.Raw, c: Id.Raw, d: Id.Raw, comptime src_loc: ?SourceLocation) void {
        ...
        pub fn new_empty() Heirarchy {
            var uninit = UNINIT;
            uninit.list = List.new_empty();
            return uninit;
        }
        ...
        pub fn ensure_unused_capacity(self: *Heirarchy, capacity: Index, alloc: AllocInfal) void {
            if (self.free_count >= capacity) return;
            const new_needed = capacity - self.free_count;
            self.list.ensure_unused_capacity(new_needed, alloc);
        }

        pub fn insert_slot_after(self: *Heirarchy, this_id: Id, alloc_else_assume_cap: ?AllocInfal) Id.Raw {
            Internal.assert_has_next(@src());
        ...
        pub fn insert_many_slots_before(self: *Heirarchy, this_id: Id.Raw, count: Index, alloc_else_assume_cap: ?AllocInfal) Range {
            Internal.assert_has_prev(@src());
            if (alloc_else_assume_cap) |alloc| self.ensure_unused_capacity(count, alloc);
            const range = Internal.initialize_items_as_siblings(self, count);
            const left_id = get_prev_id(self.get_ptr(this_id));
        ...
        pub fn insert_disconnected_items_before(self: *Heirarchy, this_id: Id.Raw, disconnected_items: Range) void {
            Internal.assert_has_prev(@src());
            Internal.assert_disconnect_range_is_isolated(self, disconnected_items, @src());
            const left_id = get_prev_id(self.get_ptr(this_id));
        ...
        pub fn insert_slot_at_beginning_of_left_children(self: *Heirarchy, parent_id: Id.Raw, alloc_else_assume_cap: ?AllocInfal) Id.Raw {
            Internal.assert_has_first_left(@src());
            if (alloc_else_assume_cap) |alloc| self.ensure_unused_capacity(1, alloc);
            const new_id = Internal.initialize_new_index(self);
            const parent_ptr = self.get_ptr(parent_id);
        ...
        pub fn insert_many_slots_at_beginning_of_left_children(self: *Heirarchy, parent_id: Id, count: Index, alloc_else_assume_cap: ?AllocInfal) Range {
            ...
            if (HAS_PARENT) {
                Internal.set_parent_on_range(self, parent_id, Id.NULL_NONE, range.first, range.last, old_first_left);
            }
            return range;
        }
        ...
        pub fn insert_disconnected_items_at_beginning_of_left_children(self: *Heirarchy, parent_id: Id, disconnected_items: Range) void {
            ...
            if (HAS_PARENT) {
                Internal.set_parent_on_range(self, parent_id, Id.NULL_NONE, disconnected_items.first, disconnected_items.last, old_first_left);
            }
        }

        pub fn insert_slot_at_end_of_left_children(self: *Heirarchy, parent_id: Id, alloc_else_assume_cap: ?AllocInfal) Id.Raw {
            Internal.assert_has_last_left(@src());
            if (alloc_else_assume_cap) |alloc| self.ensure_unused_capacity(1, alloc);
            const new_id = Internal.initialize_new_index(self);
            const parent_ptr = self.get_ptr(parent_id);
            const old_last_left = get_last_left_child_id(parent_ptr);
            const new_ptr = self.get_ptr(new_id);
            Internal.set_prev_id(new_ptr, old_last_left);
            Internal.set_last_left_child_id(parent_ptr, new_id);
            if (HAS_PARENT) {
                Internal.set_parent_id(new_ptr, parent_id);
            }
            return new_id;
        ...
        pub fn insert_many_slots_at_end_of_left_children(self: *Heirarchy, parent_id: Id, count: Index, alloc_else_assume_cap: ?AllocInfal) Range {
            Internal.assert_has_last_left(@src());
            if (alloc_else_assume_cap) |alloc| self.ensure_unused_capacity(count, alloc);
            const range = Internal.initialize_new_indexes_as_siblings(self, count);
            const parent_ptr = self.get_ptr(parent_id);
            const old_last_left = get_last_left_child_id(parent_ptr);
            const new_first_ptr = self.get_ptr(range.first);
            Internal.set_prev_id(new_first_ptr, old_last_left);
            Internal.set_last_left_child_id(parent_ptr, range.last);
            if (HAS_PARENT) {
                Internal.set_parent_on_range(self, parent_id, old_last_left, range.first, range.last, Id.NULL_NONE);
            }
            return range;
        }

        pub fn insert_disconnected_items_at_end_of_left_children(self: *Heirarchy, parent_id: Id, disconnected_items: Range) void {
            Internal.assert_has_last_left(@src());
            Internal.assert_disconnect_range_is_isolated(self, disconnected_items, @src());
            const parent_ptr = self.get_ptr(parent_id);
            const old_last_left = get_last_left_child_id(parent_ptr);
            const first_disconn_ptr = self.get_ptr(disconnected_items.first);
            Internal.set_prev_id(first_disconn_ptr, old_last_left);
            Internal.set_last_left_child_id(parent_ptr, disconnected_items.last);
            if (HAS_PARENT) {
                Internal.set_parent_on_range(self, parent_id, old_last_left, disconnected_items.first, disconnected_items.last, Id.NULL_NONE);
            }
        }

        pub fn insert_slot_at_beginning_of_right_children(self: *Heirarchy, parent_id: Id, alloc_else_assume_cap: ?AllocInfal) Id.Raw {
            Internal.assert_has_first_right(@src());
        ...
        pub fn insert_many_slots_at_beginning_of_right_children(self: *Heirarchy, parent_id: Id, count: Index, alloc_else_assume_cap: ?AllocInfal) Range {
            ...
            if (HAS_PARENT) {
                Internal.set_parent_on_range(self, parent_id, Id.NULL_NONE, range.first, range.last, old_first_right);
            }
            return range;
        }
        ...
        pub fn insert_disconnected_items_at_beginning_of_right_children(self: *Heirarchy, parent_id: Id, disconnected_items: Range) void {
            ...
            Internal.set_first_left_child_id(parent_ptr, disconnected_items.first);
            if (HAS_PARENT) {
                Internal.set_parent_on_range(self, parent_id, Id.NULL_NONE, disconnected_items.first, disconnected_items.last, old_first_right);
            }
        }

        pub fn insert_slot_at_end_of_right_children(self: *Heirarchy, parent_id: Id, alloc_else_assume_cap: ?AllocInfal) Id.Raw {
            Internal.assert_has_last_right(@src());
            if (alloc_else_assume_cap) |alloc| self.ensure_unused_capacity(1, alloc);
            const new_id = Internal.initialize_new_index(self);
            const parent_ptr = self.get_ptr(parent_id);
            const old_last_right = get_last_right_child_id(parent_ptr);
            const new_ptr = self.get_ptr(new_id);
            Internal.set_prev_id(new_ptr, old_last_right);
            Internal.set_last_right_child_id(parent_ptr, new_id);
            if (HAS_PARENT) {
                Internal.set_parent_id(new_ptr, parent_id);
            }
            return new_id;
        ...
        pub fn insert_many_slots_at_end_of_right_children(self: *Heirarchy, parent_id: Id, count: Index, alloc_else_assume_cap: ?AllocInfal) Range {
            Internal.assert_has_last_right(@src());
            if (alloc_else_assume_cap) |alloc| self.ensure_unused_capacity(count, alloc);
            const range = Internal.initialize_new_indexes_as_siblings(self, count);
            const parent_ptr = self.get_ptr(parent_id);
            const old_last_right = get_last_right_child_id(parent_ptr);
            const new_first_ptr = self.get_ptr(range.first);
            Internal.set_prev_id(new_first_ptr, old_last_right);
            Internal.set_last_right_child_id(parent_ptr, range.last);
            if (HAS_PARENT) {
                Internal.set_parent_on_range(self, parent_id, old_last_right, range.first, range.last, Id.NULL_NONE);
            }
            return range;
        }

        pub fn insert_disconnected_items_at_end_of_right_children(self: *Heirarchy, parent_id: Id, disconnected_items: Range) void {
            Internal.assert_has_last_right(@src());
            Internal.assert_disconnect_range_is_isolated(self, disconnected_items, @src());
            const parent_ptr = self.get_ptr(parent_id);
            const old_last_right = get_last_right_child_id(parent_ptr);
            const first_disconn_ptr = self.get_ptr(disconnected_items.first);
            Internal.set_prev_id(first_disconn_ptr, old_last_right);
            Internal.set_last_right_child_id(parent_ptr, disconnected_items.last);
            if (HAS_PARENT) {
                Internal.set_parent_on_range(self, parent_id, old_last_right, disconnected_items.first, disconnected_items.last,  Id.NULL_NONE);
            }
        }
        ...
        pub fn insert_slot_at_end_of_heirarchy_root(self: *Heirarchy, alloc_else_assume_cap: ?AllocInfal) Id.Raw {
        ...
        pub fn disconnect_item_range_after(self: *Heirarchy, this_id: Id, last_disconnect_id: Id) Range {
            Internal.assert_has_next(@src());
            Internal.assert_indexes_not_equal(this_id, last_disconnect_id, @src());
            const right_id = get_next_id(self.get_ptr(last_disconnect_id));
            const first_disconnect_id = get_next_id(self.get_ptr(this_id));
            Internal.remove_id_range(self, this_id, first_disconnect_id, last_disconnect_id, right_id, null);
            return Range{.first = first_disconnect_id, .last = last_disconnect_id};
        }

        pub fn disconnect_item_range_before(self: *Heirarchy, this_id: Id, first_disconnect_id: Id) Range {
            Internal.assert_has_prev(@src());
            Internal.assert_indexes_not_equal(this_id, first_disconnect_id, @src());
            const left_id = get_prev_id(self.get_ptr(first_disconnect_id));
            const last_disconnect_id = get_prev_id(self.get_ptr(this_id));
            Internal.remove_id_range(self, left_id, first_disconnect_id, last_disconnect_id, this_id, null);
            return Range{.first = first_disconnect_id, .last = last_disconnect_id};
        }
        ...
        pub fn disconnect_item_range_from_beginning_of_left_children(self: *Heirarchy, parent_id: Id, last_disconnect_id: Id) Range {
            ...
            Internal.remove_id_range(self,  Id.NULL_NONE, first_disconnect_id, last_disconnect_id, right_id, parent_id);
            return Range{.first = first_disconnect_id, .last = last_disconnect_id};
        }
        ...
        pub const Iterator = struct {
            ...
            pub inline fn get_current_id(self: *const Iterator) Id {
                if (self.frames.len == 0) return Id.NULL_NONE;
            ...
            fn goto_next(self: *Iterator) bool {
                ...
                const next_id = get_next_id(curr_ptr);
                if (next_id.is_null()) return false;
            ...
            fn goto_first_left_child(self: *Iterator) bool {
                ...
                if (left_child_id.is_null()) return false;
            ...
            fn goto_first_right_child(self: *Iterator) bool {
                ...
                if (right_child_id.is_null()) return false;
            ...
            fn init(self: *Iterator, start_id: Id) bool {
                ...
                if (start_id.is_null()) return false;
            ...
            pub fn reset_and_iterate_depth_first_and_do_actions_on_all_items(self: *Iterator, comptime actions: IteratorActions, userdata: ?*anyopaque) void {
                ...
                self.iterate_through_heirarchy_depth_first_and_do_actions_on_items(actions, false, Id.NULL_NONE, userdata);
            }
        ...
        pub const Traverser = struct {
            ...
            pub inline fn get_current_index(self: *const Traverser) Index {
                assert_with_reason(self.frames.len > 0, @src(), "no current location, traversal frames empty", .{});
                return self.frames.get_last().curr_id.idx();
            }
            ...
            pub inline fn get_parent_index(self: *const Traverser) Index {
                assert_with_reason(self.frames.len > 1, @src(), "no parent in traversal stack, either already at top level, or traverser at the same lever it was 'started' at (no parents saved above start location)", .{});
                return self.frames.ptr[self.frames.len - 2].curr_id.idx();
            }
            ...
            pub inline fn get_nth_parent_index(self: *const Traverser, n: Index) Index {
                assert_with_reason(self.frames.len > n, @src(), "no {d}-th parent in traversal stack, only {d} more parents above current", .{ n, self.frames.len -| 1 });
                return self.frames.ptr[self.frames.len - n - 1].curr_id.idx();
            }
...
const Test = if (build.mode == .Debug) struct {
    ...
    const dets = IdDetails{
        .raw_type = u32,
        .generation_bits = 16,
        .index_bits = 16,
    };
    const Id = define_id(dets);
    const IdUninit = Id{ .raw = 0xAAAAAAAA };
    ...
    const Action = struct {
        ...
        fn assert_node_before(traverser: *const List.Iterator, userdata: ?*anyopaque) bool {
            ...
            if (duplicate) |dupe| {
                t.expect_null(duplicate, "duplicate_id", "tracker {s} found duplicate id {any}", .{ tracker.name, dupe }) catch {
        ...
        fn assert_node_exit(traverser: *const List.Iterator, exit_id: List.Id, exit_kind: NodeExitKind, userdata: ?*anyopaque) bool {
            ...
            const exit_ptr: *TestElem = &traverser.heirarchy.list.ptr[exit_id.idx()];
...
test "Full_Heirarchy" {
    ...
    try Test.t.expect_equal_struct(heirarchy.first_free_id, "heirarchy.first_free_id", List.Id.NULL_NONE, "NULL_ID", "no items have been freed yet, but free list is not empty", .{});
    try Test.t.expect_equal_struct(heirarchy.first_root_id, "heirarchy.first_root_id", List.Id.NULL_NONE, "NULL_ID", "no items have been added yet, but root list is not empty", .{});
