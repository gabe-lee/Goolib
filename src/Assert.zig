...
pub inline fn generic_header(comptime BEFORE: []const u8, comptime in_comptime: bool, comptime src_loc: ?SourceLocation, comptime this: ?type, comptime log: []const u8, comptime AFTER: []const u8) []const u8 {
    ...
    return BEFORE ++ timing ++ "ERROR: " ++ ident_pfx ++ loc_prefix ++ link ++ type_chain ++ loc_func ++ ident_sfx ++ newline ++ log ++ AFTER ++ "\n";
}
...
pub inline fn err_header(comptime in_comptime: bool, comptime src_loc: ?SourceLocation, comptime this: ?type, comptime log: []const u8) []const u8 {
    return generic_header(ANSI.BEGIN ++ ANSI.FG_RED ++ ANSI.END, in_comptime, src_loc, this, log, ANSI.BEGIN ++ ANSI.RESET ++ ANSI.END);
}
...
pub inline fn warn_header(comptime in_comptime: bool, comptime src_loc: ?SourceLocation, comptime this: ?type, comptime log: []const u8) []const u8 {
    return generic_header(ANSI.BEGIN ++ ANSI.FG_YELLOW ++ ANSI.END, in_comptime, src_loc, this, log, ANSI.BEGIN ++ ANSI.RESET ++ ANSI.END);
}
