...
pub const BinarySearchResultKind = enum(u8) {
    NO_DATA_ITEMS,
    NO_SEARCH_ITEMS,
    FOUND,
...
pub fn BinarySearchOneResult(comptime IDX_TYPE: type, comptime SEARCH_CONTAINER: type) type {
    return struct {
        idx: IDX_TYPE,
        result: BinarySearchResultKind,
        updated_search: SEARCH_CONTAINER,
    };
}

pub fn LinearSearchOneResult(comptime IDX_TYPE: type, comptime SEARCH_CONTAINER: type) type {
    return struct {
        found_idx: ?IDX_TYPE,
        updated_search: SEARCH_CONTAINER,
    };
...
pub const SearchPackage = struct {
    ...
    pub fn ManyResults(comptime self: SearchPackage) type {
        return struct {
            updated_results: self.RESULT_CONTAINER,
            updated_search: self.SEARCH_PARAM_CONTAINER,
            results_count: self.RESULT_CONTAINER_IDX_TYPE,
        };
    }
    ...
    pub fn linear_search_for_one(comptime self: SearchPackage, inputs: self.SearchOneInputs()) LinearSearchOneResult(self.DATA_CONTAINER_IDX_TYPE, self.SEARCH_PARAM_CONTAINER) {
        self.assert_valid(@src());
        const DATA_IDX = self.DATA_CONTAINER_IDX_TYPE;
        if (inputs.data_start >= inputs.data_end_exclusive) return LinearSearchOneResult(self.DATA_CONTAINER_IDX_TYPE, self.SEARCH_PARAM_CONTAINER){
            .found_idx = null,
                ...
                switch (self.EQUALITY_MODE) {
                    .EXACTLY_EQUAL => {
                        if (self.exactly_equal_item(search_item, data_item, inputs.userdata)) {
                            return LinearSearchOneResult(self.DATA_CONTAINER_IDX_TYPE, self.SEARCH_PARAM_CONTAINER){
                                .found_idx = data_idx,
                    ...
                    .ORDER_EQUAL => {
                        if (self.order_match_item(search_item, data_item, inputs.userdata)) {
                            return LinearSearchOneResult(self.DATA_CONTAINER_IDX_TYPE, self.SEARCH_PARAM_CONTAINER){
                                .found_idx = data_idx,
                                .updated_search = new_search_params,
                            };
    ...
    pub fn linear_search_for_many(comptime self: SearchPackage, inputs: self.SearchManyInputs()) self.ManyResults() {
        ...
        var new_results = inputs.results;
        ...
        next_item_to_find: while (has_search_item) |search_item| {
            ...
            while (data_idx < inputs.data_end_exclusive) : (data_idx += 1) {
                ...
                const match = switch (self.EQUALITY_MODE) {
                    ...
                    .ORDER_EQUAL => self.order_match_item(search_item, data_item, inputs.userdata),
                };
                if (match) {
                    new_results = self.append_result(new_results, data_idx, inputs.results_alloc);
                    result_count += 1;
    ...
    pub fn BinaryLocateResult(comptime self: SearchPackage) type {
        return struct {
            data_idx: self.DATA_CONTAINER_IDX_TYPE,
            found: bool,

            pub fn init(initial_data_idx: self.DATA_CONTAINER_IDX_TYPE) @This() {
                return @This(){
    ...
    fn binary_locate(comptime self: SearchPackage, inputs: self.SearchInputsDataOnly(), search_item: self.SEARCH_PARAM_CONTAINER_ELEM, data_min_idx: self.DATA_CONTAINER_IDX_TYPE, data_max_idx_excluded: self.DATA_CONTAINER_IDX_TYPE) BinaryLocateResult(self) {
        var result = BinaryLocateResult(self).init(data_min_idx);
        var len = data_max_idx_excluded - data_min_idx;
        var move_right_mask: self.DATA_CONTAINER_IDX_TYPE = undefined;
        ...
        switch (self.EQUALITY_MODE) {
            ...
            .EXACTLY_EQUAL => {
                ...
                while (self.order_match_item(search_item, data_item, inputs.userdata)) {
                    result.data_idx += 1;
    ...
    /// updated search params container
    pub fn binary_search_for_one(comptime self: SearchPackage, inputs: self.SearchOneInputs()) BinarySearchOneResult(self.DATA_CONTAINER_IDX_TYPE, self.SEARCH_PARAM_CONTAINER) {
        const RESULT = BinarySearchOneResult(self.DATA_CONTAINER_IDX_TYPE, self.SEARCH_PARAM_CONTAINER);
        self.assert_valid(@src());
        if (inputs.data_end_exclusive <= inputs.data_start) {
            @branchHint(.unlikely);
            return RESULT{
                .idx = @intCast(inputs.data_start),
                .result = .NO_DATA_ITEMS,
                .updated_search = inputs.search_params,
            };
        }
        const new_search_params, const search_item_or_null = self.next_search_param_if_any(inputs.search_params);
        if (search_item_or_null) |search_item| {
            ...
            if (location.data_idx >= inputs.data_end_exclusive) {
                return RESULT{
                    .idx = @intCast(location.data_idx),
                    .result = .NOT_FOUND_ORDERED_AFTER_GIVEN_RANGE,
                    .updated_search = new_search_params,
                };
            }
            if (location.data_idx <= inputs.data_start) {
                return RESULT{
                    .idx = @intCast(location.data_idx),
                    .result = .NOT_FOUND_ORDERED_BEFORE_GIVEN_RANGE,
                    .updated_search = new_search_params,
                };
            }
            return RESULT{
                .idx = @intCast(location.data_idx),
                .result = .NOT_FOUND_ORDERED_WITHIN_RANGE,
                .updated_search = new_search_params,
            };
        } else {
            return RESULT{
                .idx = @intCast(inputs.data_start),
                .result = .NO_SEARCH_ITEMS,
                .updated_search = new_search_params,
            };
        }
    }
    ...
    pub fn binary_search_for_many(comptime self: SearchPackage, inputs: self.SearchManyInputs()) self.ManyResults() {
        ...
        if (inputs.data_end_exclusive <= inputs.data_start) return self.ManyResults(){
            .updated_results = inputs.results,
            .updated_search = inputs.search_params,
            .results_count = 0,
        };
        var result_count: RESULT_IDX = 0;
        ...
        var new_results = inputs.results;
        var new_results_end_exclusive = inputs.results_end_exclusive;
        var remaining_search_params, var has_search_item = self.next_search_param_if_any(inputs.search_params);
        while (has_search_item) |search_item| {
            ...
            if (location.found) {
                new_results = self.append_result(new_results, inputs.results_start, result_count, &new_results_end_exclusive, @intCast(location.data_idx), inputs.results_alloc);
                result_count += 1;
                if (self.SEARCH_PARAM_ORDER == .SEARCH_PARAMS_IN_SAME_ORDER_AS_THEIR_ORDER_IN_DATA_BUFFER) {
        ...
        return self.ManyResults(){
            .updated_results = inputs.results,
            .updated_search = remaining_search_params,
            .results_count = result_count - inputs.results_start,
        };
    }
    ...
    pub const ResultContainerSettings = struct {
        ...
        pub fn Appender(comptime self: @This(), comptime package: SearchPackage) type {
            return AppendFunc(self.CONTAINER_TYPE, package.DATA_CONTAINER_IDX_TYPE);
        }
    };
    ...
    pub fn set_item(comptime self: SearchPackage, data: self.DATA_CONTAINER, idx: self.DATA_CONTAINER_IDX_TYPE, val: self.DATA_CONTAINER_ELEM) self.DATA_CONTAINER {
    ...
    pub fn append_result(comptime self: SearchPackage, results: self.RESULT_CONTAINER, data_idx: self.DATA_CONTAINER_IDX_TYPE, alloc: Allocator) self.RESULT_CONTAINER {
        ...
        if (self.CUSTOM_RESULT_APPEND) |append_opaque| {
            ...
            new_results = append(results, data_idx, alloc);
        } else {
            new_results = Utils.Mem.append(new_results, data_idx, alloc);
        }
        return new_results;
    }
    ...
    pub fn order_match_item(comptime self: SearchPackage, search_item: self.SEARCH_PARAM_CONTAINER_ELEM, data_item: self.DATA_CONTAINER_ELEM, userdata: self.USERDATA_TYPE) bool {
    ...
    pub fn ResultAppender(comptime self: SearchPackage) type {
        return AppendFunc(self.RESULT_CONTAINER, self.RESULT_CONTAINER_IDX_TYPE);
    }
...
test SearchPackage {
    ...
    try Test.expect_null(empty_always_null_idx.found_idx, "empty_always_null_idx.found_idx", "fail", .{});
    const PROTO = struct {
        fn do_search_tests(buf: []u32, rand_: std.Random) anyerror!void {
            const N = buf.len;
            for (0..NUM_ITERATIONS) |_| {
                ...
                try Test.expect_null(should_not_find_idx_linear.found_idx, "should_not_find_idx_linear.found_idx", "fail", .{});
                const should_not_find_tag = if (should_not_find < buf[0]) BinarySearchResultKind.NOT_FOUND_ORDERED_BEFORE_GIVEN_RANGE else if (should_not_find > buf[N - 1]) BinarySearchResultKind.NOT_FOUND_ORDERED_AFTER_GIVEN_RANGE else BinarySearchResultKind.NOT_FOUND_ORDERED_WITHIN_RANGE;
                ...
                for (buf[0..], 0..) |should_find, i| {
                    ...
                    try Test.expect_not_null(should_find_idx_linear.found_idx, "should_find_idx_linear.found_idx", "fail", .{});
                    try Test.expect_equal(should_find_idx_linear.found_idx.?, "should_find_idx_linear.found_idx.?", i, "i", "fail", .{});
    ...
    try PROTO.do_search_tests(buf_1[0..], rand);
    try PROTO.do_search_tests(buf_8[0..], rand);
    try PROTO.do_search_tests(buf_9[0..], rand);
    //CHECKPOINT many-item tests
}
