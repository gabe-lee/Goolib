...
pub fn expect_slices_equal_t_src(comptime T: type, val_a: []const T, val_b: []const T, comptime src: SourceLocation, comptime fail_description: []const u8, fail_args: anytype) !void {
    if (!std.mem.eql(T, val_a, val_b)) {
        print_src(src, "\nFAILURE: " ++ fail_description, fail_args);
        print("\n\tEXPECT SLICES EQUAL\n\tSLICE A: {any}\n\tSLICE B: {any}\n", .{ val_a, val_b });
        return TestError.test_expected_slices_equal;
...
pub fn expect_slices_not_equal_t(comptime T: type, val_a: []const T, str_a: []const u8, val_b: []const T, str_b: []const u8, comptime fail_description: []const u8, fail_args: anytype) !void {
    if (std.mem.eql(T, val_a, val_b)) {
        print("\nFAILURE: " ++ fail_description, fail_args);
        print("\n\tEXPECT: {s} != {s}\n\tSLICE A: {any}\n\tSLICE B: {any}\n", .{ str_a, str_b, val_a, val_b });
        return TestError.test_expected_slices_equal;
...
pub fn expect_slices_not_equal_t_src(comptime T: type, val_a: []const T, val_b: []const T, comptime src: SourceLocation, comptime fail_description: []const u8, fail_args: anytype) !void {
    if (std.mem.eql(T, val_a, val_b)) {
        print_src(src, "\nFAILURE: " ++ fail_description, fail_args);
        print("\n\tEXPECT SLICES NOT EQUAL\n\tSLICE A: {any}\n\tSLICE B: {any}\n", .{ val_a, val_b });
        return TestError.test_expected_slices_equal;
    }
}
