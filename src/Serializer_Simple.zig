...
pub const ByteMoveMode = enum(u8) {};
...
pub const UnionTagType = enum(u8) {
    U8 = 1,
    U16 = 2,
    U32 = 4,
    U64 = 8,

    pub fn from_union(comptime UNION: type) UnionTagType {
        const REAL_TYPE = KindInfo.get_kind_info(UNION).UNION.tag_type.?;
        return switch (@sizeOf(REAL_TYPE)) {
            1 => UnionTagType.U8,
            2 => UnionTagType.U16,
            4 => UnionTagType.U32,
            8 => UnionTagType.U64,
            else => assert_unreachable(@src(), "tag type `{s}` is not supported", .{@typeName(REAL_TYPE)}),
        };
    }

    pub fn real_type(comptime self: UnionTagType) type {
        return switch (self) {
            .U8 => u8,
            .U16 => u16,
            .U32 => u32,
    ...
    pub fn undef(comptime self: UnionTagType) self.real_type() {
        return switch (self) {
            .U8 => 0xAA,
            .U16 => 0xAAAA,
            .U32 => 0xAAAAAAAA,
    ...
    pub fn zero(comptime self: UnionTagType) self.real_type() {
        return 0;
    }
    pub fn tag_ptr_from_union_ptr_and_offset(comptime self: UnionTagType, comptime UNION: type, union_ptr: *UNION, offset: usize) *self.real_type() {
        var raw_ptr: [*]u8 = @ptrCast(union_ptr);
        raw_ptr += offset;
        return @ptrCast(@alignCast(raw_ptr));
    }
    pub fn tag_ptr_from_union_ptr_and_offset_const(comptime self: UnionTagType, comptime UNION: type, union_ptr: *const UNION, offset: usize) *const self.real_type() {
        var raw_ptr: [*]const u8 = @ptrCast(union_ptr);
        raw_ptr += offset;
        return @ptrCast(@alignCast(raw_ptr));
    }

    pub fn from_serial_slice(comptime self: UnionTagType, data: []const u8) self.real_type() {
    ...
    pub fn cast_union_tag(comptime self: UnionTagType, any_union: anytype) self.real_type() {
        const tag = std.meta.activeTag(any_union);
        return @bitCast(tag);
    }
};

pub const ByteUnionHeader = struct {
    num_fields: u16,
    tag_type: UnionTagType,
};

pub const ByteUnionTagRoutineOffset = struct {
...
pub const CustomSerializeFn = fn (comptime self: *ByteMoveBuilder, comptime alloc: Allocator, comptime curr_native_offset_: i32, comptime TARGET_ENDIAN: Endian) void;
pub const CustomSerializeFnName = "custom_serialize";
...
pub const ByteMoveBuilder = struct {
    moves: List(ByteMove) = .{},
    curr_serial_offset: i32 = 0,

    pub fn ensure_space_for_n_more_bytes(comptime self: *ByteMoveBuilder, comptime n: usize, comptime alloc: Allocator) void {
        self.moves.ensure_free_slots(n, alloc);
    }
    pub fn add_endian_bytes(comptime self: *ByteMoveBuilder, comptime curr_native_offset_: i32, comptime size: usize, comptime TARGET_ENDIAN: Endian, comptime alloc: Allocator) void {
        self.ensure_space_for_n_more_bytes(size, alloc);
        self.add_endian_bytes_assume_capacity(curr_native_offset_, size, TARGET_ENDIAN);
    }
    pub fn add_endian_bytes_assume_capacity(comptime self: *ByteMoveBuilder, comptime curr_native_offset_: i32, comptime size: usize, comptime TARGET_ENDIAN: Endian) void {
        ...
        if (SWAP) {
            ...
            while (local_byte_idx < size) : (local_byte_idx += 1) {
                ...
                _ = self.moves.append_assume_capacity(.new(native_to_serial_delta));
    ...
    pub fn add_type_with_custom_serializer(comptime self: *ByteMoveBuilder, comptime alloc: Allocator, comptime curr_native_offset_: i32, comptime TARGET_ENDIAN: Endian, comptime TYPE: type) void {
        assert_with_reason(type_has_custom_serialize(TYPE), @src(), "type `{s}` does not have a custom serialize function", .{@typeName(TYPE)});
        comptime @call(.auto, @field(TYPE, CustomSerializeFnName), .{ self, alloc, curr_native_offset_, TARGET_ENDIAN });
    }

    pub fn add_type(comptime self: *ByteMoveBuilder, comptime alloc: Allocator, comptime curr_native_offset_: i32, comptime TARGET_ENDIAN: Endian, comptime TYPE: type) void {
        const INFO = KindInfo.get_kind_info(TYPE);
        const SIZE = @sizeOf(TYPE);
        comptime var curr_native_offset: i32 = curr_native_offset_;
        re_typed: switch (INFO) {
            .INT, .FLOAT, .BOOL, .ENUM => {
                self.add_endian_bytes(curr_native_offset, @sizeOf(TYPE), TARGET_ENDIAN, alloc);
            },
            .ARRAY, .VECTOR => {
                ...
                for (0..LEN) |_| {
                    self.add_type(alloc, curr_native_offset, TARGET_ENDIAN, CHILD);
                    curr_native_offset += SIZE;
                }
            },
            .STRUCT => |S| {
                ...
                if (comptime type_has_custom_serialize(TYPE)) {
                    self.add_type_with_custom_serializer(alloc, curr_native_offset, TARGET_ENDIAN, TYPE);
                } else {
                    inline for (S.fields) |field| {
                        const local_offset = @offsetOf(TYPE, field.name);
                        const real_offset = curr_native_offset + local_offset;
                        self.add_type(alloc, real_offset, TARGET_ENDIAN, field.type);
                    }
                }
            },
            .UNION => |U| {
                if (comptime type_has_custom_serialize(TYPE)) {
                    ...
                    assert_with_reason(U.tag_type != null, @src(), "cannot automatically serialize an untagged union", .{});
                    // CHECKPOINT union routine
                    // 0: make a new union type with ByteMove, ByteUnionHeader, and ByteUnionTagRoutineOffset
                    // 1: add ByteUnionHeader
                    // 2: add bytes for union `tag` value
                    // 3: reserve the next `tag_count + 1` elements for ByteUnionTagRoutineOffset's
                    //      3.1: retain a slice of those reserved ByteUnionTagRoutineOffset's
                    //      3.2: start a new routine delta var from `tag_count + 1`
                    // 4: for each possible tag (in field order, index `i`):
                    //      4.0: cache `curr_serial_offset` as `prev_serial_offset`
                    //      4.1: slice from 3.1 index `i` = delta from 3.2 - `i`
                    //      4.2: call `add_type()` on field type with the unaltered `curr_native_offset`
                    //      4.3: delta from 3.2 += `curr_serial_offset` - `prev_serial_offset`
                    // 5: slice from 3.1 index `tag_count` = delta from 3.2 - `tag_count`
                    // ...
                    // X: maybe this even still isnt possible? use HybridUnion?
                }
            },
